...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        self.gt_encoder_depth = gt_encoder_depth
    ...
    def forward_core(self, x, t, y):
        ...
        if self.learn_sigma:
            x, _ = x.chunk(2, dim=1)
        return x, (zs, gt_zs, intermediate_activations)

    def forward(self, x, t, y):
        return self.forward_core(x, t, y)

    def forward_with_hook(self, x, t, y):
        """
        Forward pass of SiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        ...
        for idx, block in enumerate(self.blocks):
            hook_name = f"block_{idx}"
            hooks[hook_name] = block.register_forward_hook(create_hook(hook_name))
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        return torch.cat([eps, rest], dim=1), None
